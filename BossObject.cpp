...
void BossObject :: Show(SDL_Renderer* des)
{
	...
	if (think_time_==0)
	{
		...
		SDL_Rect* CreentClip = &frame_clip_[frame_];
		SDL_Rect RenderQuad = {rect_.x, rect_.y, width_frame_, height_frame_};
		if (CreentClip!=NULL)
		{
			RenderQuad.w=CreentClip->w;
			RenderQuad.h=CreentClip->h;
		}
		SDL_RenderCopy(des, p_object_, CreentClip, &RenderQuad);
	}
}
void BossObject :: DoPlayer(Map& g_map)
{
	if (think_time_ == 0)
	{
		...
		if (input_type_.jump_==1)
		{
			if(on_ground_==1)
			{
				y_val_ = -PLAYER_HIGHT_VAL; // CHUA HIEU
...
{
	g_map.stat_x_ = x_pos_ - (SCREEN_WIDTH/2);
	if (g_map.stat_x_ < 0) 
	{
		g_map.stat_x_ = 0;
	}
	else if (g_map.stat_x_ + SCREEN_WIDTH >= g_map.max_x_)
	{
		g_map.stat_x_ = g_map.max_x_-SCREEN_WIDTH;
	}
	g_map.stat_y_ = y_pos_ - (SCREEN_HEIGHT/2);
	if (g_map.stat_y_ < 0)
	{
		g_map.stat_y_ = 0;
	}
	else if (g_map.stat_y_ + SCREEN_HEIGHT >= g_map.max_y_)
	{
		g_map.stat_y_ = g_map.max_y_ - SCREEN_HEIGHT;
	}
}
void BossObject :: CheckToMap(Map& g_map)
{
	...
	y2 = (y_pos_ + height_frame_ + y_val_) / TILE_SIZE; // -1
	if (x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
	{
		if (y_val_ > 0)
		{
	...
	{
		think_time_ = 60;  // thoi gian tro lai la 60;
	}
}
